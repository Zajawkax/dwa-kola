...
function EditBike() {
    ...
    useEffect(() => {
        axios
            .get(`/api/bikes/${id}`)
            .then((response) => {
                const productionDate = new Date(response.data.productionDate).toISOString().split('T')[0];
                setBike({ ...response.data, productionDate });
            })
            .catch((error) => {
    ...
    const handleSubmit = (e) => {
        ...
        axios
            ...
            .catch((error) => {
                console.error('Błąd podczas aktualizacji samochodu:', error.response.data);
            });
    };
